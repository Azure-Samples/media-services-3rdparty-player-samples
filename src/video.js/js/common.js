...
class BasePlayer {
  constructor () {
    ...
    this.sastoken = this.getUrlParameter('sastoken')
  ...
  parseStreamingSettingsToText () {
    ...
    return `${version}<br>${plugins}<br>${format}<br>${protection}<br>${authorizationToken}<br>${sasToken}`
  }
  ...
  initPage () {
    ...
    this.$('sastokenCheckbox').addEventListener('click', this.configureSasToken.bind(this))
    ...
    if (this.sastoken) {
      this.$('sastokenInput').value = this.sastoken
      this.$('sastokenCheckbox').checked = true
      this.configureSasToken()
    }
  ...
  configureSasToken () {
    this.$('sastokenInput').disabled = !this.$('sastokenCheckbox').checked

    if (this.$('sastokenInput').disabled) {
      this.$('sastokenInput').value = ''
    }
  }
  ...
  getInputSasToken () {
    return this.$('tokenSasInput').value
  }
